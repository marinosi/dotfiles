...
grmlcomp() {
    ...
    if is42 ; then
        [[ -r ~/.ssh/config ]] && _ssh_config_hosts=(${${${${(f)"$(egrep "^Host " $HOME/.ssh/config | fgrep -v '*' | awk '{print $2}')"}:#[\|]*}%%\ *}%%,*}) || _ssh_config_hosts=()
...
#f5# Make screenshot
sshot() {
    [ -z "$SSHOTDIR" ] && SSHOTDIR="/Users/marinosi/Pictures/ScreenShots"
    if isdarwin ; then 
        [[ ! -d $SSHOTDIR ]] && mkdir -p ${SSHOTDIR}
        ...
        SSHOTNAME="${SSHOTDIR}/`date "+%Y-%m-%d--%H:%M:%S"`.jpg"
        screencapture $SSHOTNAME
        print "Screenshot taken : ${SSHOTNAME}"
    fi
}
